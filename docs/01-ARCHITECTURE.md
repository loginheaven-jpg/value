...
1단계 분류 상태는 별도 키 `values-triage`에 둡니다(라운드·남은 큐·카드별 판단·되돌리기 이력·국면).
`values-progress`와 나누어 두는 이유는 두 상태의 수명이 다르기 때문입니다 — 분류는 라운드를 넘어
누적되고, 진행 상태는 단계마다 갈아 끼워집니다. 복원 시 저장된 큐에서 현재 데이터에 없는 id는
걸러냅니다. 두 키 모두 24시간 만료이며 3단계 종료·결과 화면 초기화·이력 화면 초기화에서 함께 지웁니다.
