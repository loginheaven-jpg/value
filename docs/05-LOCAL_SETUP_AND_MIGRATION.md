...
위 표 중 브라우저가 있어야 잴 수 있는 것들은 자동화돼 있습니다.

```bash
pnpm build && node scripts/e2e-check.mjs
```

Playwright 가 있으면(전역·npx 캐시 포함) 진짜 크로미움으로 7항목을 재고, 없으면 조용히
건너뜁니다(exit 0). CI 에서 강제하려면 `E2E_REQUIRED=1` 을 줍니다. **의존성으로 넣지
않았습니다** — 설치 용량이 크고, 이 점검이 매 커밋마다 필요하지는 않습니다.

재는 항목: 인쇄 시 접힌 성찰 질문 노출 · 카드 페이지 분할 · 375px 분류 화면 · 키보드
(화살표·Backspace·Alt 조합) · 인쇄 버튼 폭 은닉과 모바일 복사 안내 · 검토 화면의 더미 복귀 ·
비로그인 관리자 화면의 데이터 미노출.

라우터 수준의 권한 검증은 브라우저 없이 `server/adminLock.test.ts` 가 실제 호출로 잽니다.
